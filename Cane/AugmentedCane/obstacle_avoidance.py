...
hallway_steering_thresh = 950.0 # if any lidar reading less than this, move away from that side
forward_thresh = 1850.0 # only look to turn if less than 2 meters of room ahead
...
angle_offset = 0.0 #-45.0 # because lidar turned 45 degrees to left
...
while(run_flag):
    ...
    while(button_state): # run until reset button pushed
        ...
        if distances[mid_ind] > 0: # check if lidar readings initialized
            ## CHECK STATE MACHINE
            playing_audio, motor_command = cf.obstacle_state_machine(playing_audio, distances, mid_ind, m_offset, motor_gain, motor_max, deadband, target_heading, current_heading, motor_min, lidar_arr_mean, forward_thresh)
